...
export default function MissionariesByAssignmentLocationsPage() {
  ...
  // Search form state
  const [assignmentLocationIds, setAssignmentLocationIds] = useState<string>('');
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('missionariesByAssignmentLocationsSearchHistory');
    if (savedHistory) {
      try {
  ...
  const saveSearchHistory = (history: SearchHistory[]) => {
    localStorage.setItem('missionariesByAssignmentLocationsSearchHistory', JSON.stringify(history));
    setSearchHistory(history);
  };

  const addToSearchHistory = (ids: string[], resultsCount: number) => {
    const newEntry: SearchHistory = {
      timestamp: new Date().toISOString(),
      assignmentLocationIds: ids,
      resultsCount
    };
  ...
  const clearSearchHistory = () => {
    localStorage.removeItem('missionariesByAssignmentLocationsSearchHistory');
    setSearchHistory([]);
  };

  const parseAssignmentLocationIds = (input: string): string[] => {
    return input
      .split(/[,\s\n]+/)
      .map(id => id.trim())
  ...
  const searchMissionariesByAssignmentLocations = async () => {
    if (!apiClient) {
      setError('API client not initialized. Please refresh the page.');
      return;
    }

    const ids = parseAssignmentLocationIds(assignmentLocationIds);
    if (ids.length === 0) {
      setError('Please provide at least one Assignment Location ID to search for missionaries.');
      return;
    }

    if (ids.length > 20) {
      setError('Please limit your search to 20 or fewer Assignment Location IDs at once.');
      return;
    }
    ...
    try {
      const query = `
        query MissionariesByAssignmentLocations($ids: [ID]) {
          missionariesByAssignmentLocations(ids: $ids) {
            missionaryNumber
            cmisUUID
            id
            legacyCmisId
      ...
      const response = await apiClient.executeGraphQLQuery(query, { ids });
      
      if (response.errors && response.errors.length > 0) {
        throw new Error(response.errors[0].message);
      }

      const data = response.data as { missionariesByAssignmentLocations: Missionary[] };
      const missionaryList = Array.isArray(data.missionariesByAssignmentLocations) 
        ? data.missionariesByAssignmentLocations 
        : [];
      setMissionaries(missionaryList);
      addToSearchHistory(ids, missionaryList.length);
      
      if (missionaryList.length === 0) {
        setError(`No missionaries found for the provided Assignment Location IDs: ${ids.join(', ')}`);
      }
    } catch (err: any) {
      console.error('Error searching missionaries by assignment locations:', err);
      setError(err.message || 'Failed to search missionaries by assignment locations');
      addToSearchHistory(ids, 0);
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCsv = () => {
    ...
    const csvContent = [
      ...
      ...sortedAndFilteredMissionaries.map(missionary => [
        ...
        missionary.assignments?.length || 0
      ].join(','))
    ].join('\n');
    
    const assignmentLocationIdsStr = parseAssignmentLocationIds(assignmentLocationIds).join('-');
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `missionaries-by-locations-${assignmentLocationIdsStr}-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const repeatSearch = (historyItem: SearchHistory) => {
    setAssignmentLocationIds(historyItem.assignmentLocationIds.join(', '));
  };

  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      searchMissionariesByAssignmentLocations();
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Missionaries by Assignment Locations</h1>
          <p className="mt-2 text-lg text-gray-600">
            Find all missionaries assigned to multiple assignment locations by providing multiple Assignment Location IDs
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="assignmentLocationIds" className="block text-sm font-medium text-gray-700 mb-2">
                Assignment Location IDs <span className="text-red-500">*</span>
              </label>
              <textarea
                id="assignmentLocationIds"
                value={assignmentLocationIds}
                onChange={(e) => setAssignmentLocationIds(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder="Enter assignment location IDs separated by commas, spaces, or new lines...&#10;Example: 12345, 67890, 11223"
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
              <p className="mt-1 text-xs text-gray-500">
                Enter multiple assignment location IDs (max 20) separated by commas, spaces, or new lines
              </p>
            </div>

            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">
                {assignmentLocationIds.trim() && (
                  <span>
                    {parseAssignmentLocationIds(assignmentLocationIds).length} Assignment Location ID(s) to search
                  </span>
                )}
              </div>
              
              <button
                onClick={searchMissionariesByAssignmentLocations}
                disabled={loading || !assignmentLocationIds.trim()}
                  ...
                  <div className="flex items-center space-x-3 flex-1">
                    <div className="text-sm flex-1">
                      <div className="font-medium text-gray-900">
                        Assignment Location IDs: <span className="text-blue-600">{item.assignmentLocationIds.slice(0, 3).join(', ')}</span>
                        {item.assignmentLocationIds.length > 3 && <span className="text-gray-500"> +{item.assignmentLocationIds.length - 3} more</span>}
                      </div>
                    </div>
        ...
        {missionaries.length > 0 && (
          <>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">
                    Missionaries for Assignment Locations: {sortedAndFilteredMissionaries.length} results
                  </h2>
                  <p className="text-gray-600">
                    Missionaries assigned to {parseAssignmentLocationIds(assignmentLocationIds).length} assignment location(s)
                  </p>
                </div>
                            ...
                            {missionary.assignments && missionary.assignments.length > 0 ? (
                              missionary.assignments.slice(0, 3).map((assignment, assignmentIndex) => (
                                <div key={assignmentIndex} className="bg-gray-50 p-2 rounded text-xs">
                                  ...
                                  {assignment.component?.assignmentLanguage?.languageName && (
                                    <div className="text-blue-600">
                                      Lang: {assignment.component.assignmentLanguage.languageName}
                                    </div>
                                  )}
            ...
            <div className="mt-8 bg-gray-50 border border-gray-200 rounded-lg p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">📊 Assignment Locations Summary Statistics</h3>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-purple-100 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-purple-900">
                    {parseAssignmentLocationIds(assignmentLocationIds).length}
                  </div>
                  <div className="text-sm text-purple-700">Assignment Locations</div>
                </div>
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h3 className="text-lg font-medium text-blue-900 mb-3">💡 How to Use Missionaries by Assignment Locations Search</h3>
          <div className="text-sm text-blue-800 space-y-2">
            <p>• <strong>Multiple IDs:</strong> Enter multiple assignment location IDs separated by commas, spaces, or new lines (max 20)</p>
            <p>• <strong>Bulk Search:</strong> Find all missionaries assigned to any of the specified assignment locations</p>
            <p>• <strong>Assignment Details:</strong> View detailed assignment information including mission names and languages</p>
